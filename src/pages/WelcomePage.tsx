...
// These can be configured in Admin Panel -> Settings -> App Settings
const DEFAULT_CHANNEL_USERNAME = "karaeski_official";
const DEFAULT_BOT_TOKEN = "YOUR_BOT_TOKEN";
  ...
  const checkChannelMembership = async () => {
    ...
    try {
      ...
      if (!userId) {
        toast({
          title: "Hata",
          description: "Telegram kullanıcı bilgisi alınamadı",
          variant: "destructive",
        });
        setIsChecking(false);
        return;
      ...
      if (data.ok) {
        ...
        if (isMember) {
          ...
          toast({
            title: "✅ Doğrulandı!",
            description: "Kanala katılımınız onaylandı. Hoş geldiniz!",
          });
          ...
          toast({
            title: "Henüz katılmadınız",
            description: "Lütfen önce kanala katılın, sonra tekrar kontrol edin",
            variant: "destructive",
          });
      ...
      // For demo purposes, if bot token is not configured, allow after 2 seconds
      if (telegramConfig.botToken === "YOUR_BOT_TOKEN") {
        ...
        toast({
          ...
          description: "Lütfen tekrar deneyin veya destek ile iletişime geçin",
          variant: "destructive",
        });
      }
    } finally {
        ...
        <div className="p-8 space-y-6 relative z-10">
          ...
          <div className="text-center space-y-2 animate-slide-up animation-delay-200">
            ...
            <p className="text-slate-300 text-sm">
              Premium Casino & Betting Platform
            </p>
          </div>
            ...
            {!hasJoined ? (
              <>
                <Button 
                  ...
                  className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-semibold py-6 text-lg shadow-lg hover:shadow-emerald-500/50 transition-all duration-300 hover:scale-105"
                >
                ...
                <Button 
                  ...
                  disabled={isChecking}
                  variant="outline"
                  className="w-full border-emerald-500/30 hover:bg-emerald-500/10 text-emerald-400 font-semibold py-6 text-lg transition-all duration-300 hover:scale-105"
                >
                  {isChecking ? (
                    <>
              ...
              <Button 
                onClick={onComplete}
                className="w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white font-semibold py-6 text-lg shadow-lg hover:shadow-green-500/50 transition-all duration-300 hover:scale-105"
              >
                <CheckCircle2 className="mr-2 h-5 w-5" />
                Uygulamaya Başla
              </Button>
          ...
          {/* Footer */}
          <p className="text-center text-slate-500 text-xs animate-slide-up animation-delay-500">
            Karaeski topluluğuna hoş geldiniz! 🎉
          </p>
        </div>
      </Card>
    </div>
